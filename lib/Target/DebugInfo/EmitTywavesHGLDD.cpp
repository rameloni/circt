//===- EmitTywavesHGLDD.cpp - HGLDD debug info emission
//--------------------------===//
//
...
// ===- The file format to emit - snake_case for consistency --------------===//
// struct Scope {
//     name:            String,         // name of the scope (equivalent to a
//     module/instance) child_variables: Vec<Variable>,  // variables declared
//     in this scope (ports, regs, wires) child_scopes:    Vec<Scope>,     //
//     nested scopes (modules, instances)
// }
//
// struct Variable {
//     name:      String,               // name of the variable (in the source
//     language) type_name: String,               // name of the type of the
//     variable (in the source language) hw_type:   HwType,               //
//     hardware type of the variable real_type: RealType,             // real
...
//     Wire,
//     Reg,
//     Port { direction: Direction },   // A port has also a direction
//     Mem,
//     Unknown,
// }
//
...
// }
// ===----------------------------------------------------------------------===//
namespace hgldd::keywords {
static constexpr auto SCOPES = "scopes";
static const struct scope {
  static constexpr auto NAME = "name";
  static constexpr auto TYPE_NAME = "type_name";
  static constexpr auto VERILOG_NAME = "verilog_name";
  static constexpr auto CHILD_VARIABLES = "child_variables";
  static constexpr auto CHILD_SCOPES = "child_scopes";

  static const struct variable {
    static constexpr auto NAME = "name";
    static constexpr auto TYPE_NAME = "type_name";
    static constexpr auto HW_TYPE = "hw_type";
    static constexpr auto REAL_TYPE = "real_type";
    static constexpr auto PARAMS = "params";

    static const struct hw_type {
      static constexpr auto WIRE = "wire";
    ...
    static const struct real_type {
      static constexpr auto GROUND = "ground";
      static constexpr auto VEC = "vec";
      static constexpr auto BUNDLE = "bundle";
      static constexpr auto UNKNOWN = "unknown";

      static const struct ground {
        static constexpr auto WIDTH = "width";
        static constexpr auto VCD_NAME = "vcd_name";
      } ground;
      static const struct vec {
        static constexpr auto SIZE = "size";
        static constexpr auto FIELDS = "fields";
      } vec;
      static const struct bundle {
        static constexpr auto FIELDS = "fields";
        static constexpr auto VCD_NAME = "vcd_name";
      } bundle;
...
struct EmittedType {
  ...
  static JArray emitDims(ArrayRef<int64_t> dims, bool skipFirstLen1Dim) {
    ...
    if (skipFirstLen1Dim && !dims.empty() &&
        dims[0] == 0) // TODO: check this 0-width
...
struct FileEmitter {
  ...
  EmittedExpr emitExpression(Value value, llvm::StringRef varName, std::string tabs = "");
...
void FileEmitter::emit(JOStream &json) {
  ...
  json.attributeArray(hgldd::keywords::SCOPES, [&] {
    // TODO: in my file format I don't need this
...
void FileEmitter::emitModule(JOStream &json, DIModule *module) {
  // TODO: in my file format this is the equivalent of an emit scope
  moduleNamespace = state.objectNamespace;
  structNameHint = module->name.getValue();
  json.objectBegin();
  json.attribute("kind", "module");
  json.attribute(hgldd::keywords::scope::NAME, getModuleName(module)); // HGL
  json.attribute(hgldd::keywords::scope::VERILOG_NAME,
                 getVerilogModuleName(*module).getValue()); // HDL
  json.attribute(hgldd::keywords::scope::TYPE_NAME, "na");
  if (module->isExtern)
    json.attribute("isExtModule", 1);
  // TODO: no location in my file format
  if (auto *op = module->op) {
...
void FileEmitter::emitModuleBody(JOStream &json, DIModule *module) {
  // TODO: this is the equivalent of creating child_variables and child_scopes
  json.attributeArray(hgldd::keywords::scope::CHILD_VARIABLES, [&] {
    for (auto *var : module->variables)
      emitVariable(json, var);
  });
  json.attributeArray(hgldd::keywords::scope::CHILD_SCOPES, [&] {
...
JValue createVariableEntry(const StringRef &name, const StringAttr &typeName,
                           const ArrayAttr &params,
                           const EmittedExpr &emittedExpr) {

  JObject varObj; // The object to be returned

  if (!emittedExpr)
    return varObj;

  // 1. Name
  varObj[hgldd::keywords::scope::variable::NAME] = name;
  // 2. The source type name
  if (typeName)
    varObj[hgldd::keywords::scope::variable::TYPE_NAME] = typeName.getValue();
  // 3. Constructor parameters of source type (if any)
  if (params && !(params).empty()) {
    varObj[hgldd::keywords::scope::variable::PARAMS] = [&params]() {
      auto jArr = JArray{};
      for (const auto &param : params) {
        if (isa<DictionaryAttr>(param)) {
          // Build the JObject from the dictionary
          auto jobj = JObject{};
          for (const auto &x : cast<DictionaryAttr>(param).getValue()) {
            jobj[x.getName().getValue()] =
                cast<StringAttr>(x.getValue()).getValue();
          }
          jArr.push_back(std::move(jobj));
        }
      }
      return jArr;
    }();
  ...
  // 4. Hardware type
  varObj[hgldd::keywords::scope::variable::HW_TYPE] = emittedExpr.type.name;
  // 5. Real type of the variable
  JObject realTypeObj;
  // Get the packed_range: width_range
  realTypeObj = *emittedExpr.expr.getAsObject();
  if (auto dims = emittedExpr.type.emitPackedDims();
      !dims.empty() && emittedExpr.type.name != "struct" &&
      emittedExpr.type.name != "array")
    realTypeObj[hgldd::keywords::scope::variable::real_type::ground::WIDTH] =
        std::move(dims);

  if (emittedExpr.type.name == "struct" || emittedExpr.type.name == "array") {
    varObj[hgldd::keywords::scope::variable::REAL_TYPE] =
        std::move(realTypeObj);
  } else {
    varObj[hgldd::keywords::scope::variable::REAL_TYPE] =
        JObject{{hgldd::keywords::scope::variable::real_type::GROUND,
                 std::move(realTypeObj)}};
  }
  return varObj;
}
...
void FileEmitter::emitInstance(JOStream &json, DIInstance *instance) {
  ...
  LLVM_DEBUG(llvm::dbgs() << "ok");
  json.attribute(hgldd::keywords::scope::NAME, instanceName);
...
/// Emit the debug info for a `DIVariable`.
/// Alias: variable of child_variables
void FileEmitter::emitVariable(JOStream &json, DIVariable *variable) {

  if (auto value = variable->value)
    json.rawValue([&](auto &os) {
      os << createVariableEntry(
          legalizeInModuleNamespace(variable->name),
          variable->sourceLangType.typeName, variable->sourceLangType.params,
          emitExpression(value, variable->name.getValue()));
    });
}

/// Emit the DI expression necessary to materialize a value.
/// Alias: real_type
EmittedExpr FileEmitter::emitExpression(Value value, llvm::StringRef varName,
                                        std::string tabs) {

  auto verilogGroundName = [](StringRef sigName) -> JObject {
    return JObject{
        {hgldd::keywords::scope::variable::real_type::ground::VCD_NAME,
         sigName}};
  };
  ...
  if (auto blockArg = dyn_cast<BlockArgument>(value)) {
    ...
    if (!name)
      return {};
    return {verilogGroundName(name), value.getType()};
  }
  ...
  LLVM_DEBUG(llvm::dbgs() << tabs << "Emitting expression: result: " << result
                          << "\n");

  // If the operation is a named signal in the output Verilog,
  if (isa<hw::WireOp, sv::WireOp, sv::RegOp, sv::LogicOp>(op)) {
    auto name = op->getAttrOfType<StringAttr>("hw.verilogName");
    LLVM_DEBUG(llvm::dbgs()
               << tabs << "Emitting named signal: " << name << "\n");
    if (!name || name.empty())
      name = op->getAttrOfType<StringAttr>("name");
    if (name && !name.empty())
      return {verilogGroundName(name),
              result.getType()}; // TODO: check this result.getType
  }
  ...
  if (auto constOp = dyn_cast<hw::ConstantOp>(op)) {
    LLVM_DEBUG(llvm::dbgs()
               << tabs << "Emitting constant: " << constOp << "\n");
    ...
    // Pad the string with leading zeros such that it is exactly of the
    // required width. This is needed since tools will use the string length
    // to determine the width of the constant.
    ...
    return {JObject({{"constant", buffer}}), type};
  }

  // Emit structs as => bundle: { fields: [ ... ], vcd_name: "..." }
  if (auto structOp = dyn_cast<debug::StructOp>(op)) {
    LLVM_DEBUG(llvm::dbgs()
               << tabs << "Emitting dbg struct: " << structOp << "\n");
    // Collect the fields
    std::vector<JValue> fields;
    for (auto [nameAttr, field] :
         llvm::zip(structOp.getNamesAttr(), structOp.getFields())) {
      // Emit the field expression.
      if (auto fieldVariable =
              emitExpression(field, std::string(tabs + "\t"))) {
        fields.push_back(fieldVariable.expr);
      }
    }
    ...
    //   return {};
    // }
    LLVM_DEBUG(llvm::dbgs() << tabs << "---- RETURNING: fields \n");
    JObject structObj;
    structObj[hgldd::keywords::scope::variable::real_type::bundle::VCD_NAME] =
        varName;
    structObj[hgldd::keywords::scope::variable::real_type::bundle::FIELDS] =
        std::move(fields);
    // TODO: vcd_name if any
    // if (!fields.empty()) {

    // }

    return {JObject{{hgldd::keywords::scope::variable::real_type::BUNDLE,
                     std::move(structObj)}},
            EmittedType("struct")};
  }

  // Emit arrays as => vec: { size: N, fields: [ ... ] }
  if (auto arrayOp = dyn_cast<debug::ArrayOp>(op)) {
    LLVM_DEBUG(llvm::dbgs()
               << tabs << "Emitting dbg array: " << arrayOp << "\n");
    std::vector<JValue> fields;
    EmittedType type;
    // Emit an expression for each element of the array.
    for (auto element : arrayOp.getElements()) {
      if (auto fieldVariable =
              emitExpression(element, std::string(tabs + "\t"))) {
        fields.push_back(fieldVariable.expr);
        if (type && type != fieldVariable.type)
          return {};
        type = fieldVariable.type;
      }
    }

    type.addUnpackedDim(fields.size());
    type.name = "array"; // TODO: check this

    auto arrrayObj = JObject{
        {hgldd::keywords::scope::variable::real_type::vec::SIZE, fields.size()},
        {hgldd::keywords::scope::variable::real_type::vec::FIELDS,
         std::move(fields)}};
    return {JObject{{hgldd::keywords::scope::variable::real_type::VEC,
                     std::move(arrrayObj)}},
            type};
  }
  // Emit subvar as =>
  ...
  if (auto subVarOp = dyn_cast<debug::SubFieldOp>(op)) {
    LLVM_DEBUG(llvm::dbgs() << tabs << "Emitting subvar: " << subVarOp << "\n");

    if (auto value = subVarOp.getValue()) {
      if (auto emittedExpr = emitExpression(value, std::string(tabs + "\t"))) {
        return {createVariableEntry(subVarOp.getName(),
                                    subVarOp.getTypeNameAttr(),
                                    subVarOp.getParamsAttr(), emittedExpr),
                emittedExpr.type};
      }
    }

    // It is a ground? Yes probably
    return {};
  }

  if (auto readOp = dyn_cast<sv::ReadInOutOp>(op))
    return emitExpression(readOp.getInput(), std::string(tabs + "\t"));
  ...
  if (!unaryOpcode.empty() && op->getNumOperands() == 1) {
    auto arg = emitExpression(op->getOperand(0), varName);
    if (!arg)
      return {};
  ...
  if (!binaryOpcode.empty()) {
    ...
    auto lhs = emitExpression(op->getOperand(0), varName);
    auto rhs = emitExpression(op->getOperand(1), varName);
    if (!lhs || !rhs)
      return {};
  ...
  if (auto concatOp = dyn_cast<comb::ConcatOp>(op)) {
    if (auto concatOp = dyn_cast<comb::ConcatOp>(op)) {
      ...
      for (auto operand : concatOp.getOperands()) {
        auto value = emitExpression(operand, varName);
        if (!value)
          return {};
  ...
  if (auto replicateOp = dyn_cast<comb::ReplicateOp>(op)) {
    if (auto replicateOp = dyn_cast<comb::ReplicateOp>(op)) {
      auto arg = emitExpression(replicateOp.getInput(), varName);
      if (!arg)
        return {};
  ...
  if (auto extractOp = dyn_cast<comb::ExtractOp>(op)) {
    auto arg = emitExpression(extractOp.getInput(), varName);
    if (!arg)
      return {};
  ...
  if (auto muxOp = dyn_cast<comb::MuxOp>(op)) {
    auto cond = emitExpression(muxOp.getCond(), varName);
    auto lhs = emitExpression(muxOp.getTrueValue(), varName);
    auto rhs = emitExpression(muxOp.getFalseValue(), varName);
    if (!cond || !lhs || !rhs)
      return {};
  ...
  for (auto &use : result.getUses()) {
    ...
    if (auto wireOp = dyn_cast<hw::WireOp>(user))
      if (wireOp.getInput() == result)
        return emitExpression(wireOp, varName);
    ...
    if (auto assignOp = dyn_cast<sv::AssignOp>(user))
      if (assignOp.getSrc() == result)
        return emitExpression(assignOp.getDest(), varName);
    ...
    if (isa<hw::OutputOp>(user)) {
      ...
      auto portName = mod.getPort(mod.getHWModuleType().getPortIdForOutputId(
                          ...
                          .getVerilogName();
      return {verilogGroundName(portName), result.getType()};
    }
    // TODO
  }
...
/// Contextual information for HGLDD emission shared across multiple HGLDD
/// files. This struct is used to determine an initial split of debug info
/// files and to distribute work.
struct Emitter {
  GlobalState state;
...
Emitter::Emitter(Operation *module, const EmitHGLDDOptions &options)
  ...
  // group. That group, with a null `StringAttr` key, is emitted into a
  // separate "global.dd" file.
...
LogicalResult debug::emitTywavesHGLDD(Operation *module, llvm::raw_ostream &os,
                                      const EmitHGLDDOptions &options) {
...
LogicalResult debug::emitSplitTywavesHGLDD(Operation *module,
                                           const EmitHGLDDOptions &options) {
